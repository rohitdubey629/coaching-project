                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody id="call_record">
                                            <?php
                                            if (!empty($quickly_enquiry)) {
                                                $styleClass = array("table-warning", "table-success");
                                                $i = 0;
                                                foreach ($quickly_enquiry as $val) {
                                                    if ($i % 2 == 0) {
                                                        $style = $styleClass['1'];
                                                    } else {
                                                        $style = $styleClass['0'];
...
<script>
    function filterData() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('quickly/filterData'); ?>",
            data: {
                date_from: date_from,
                date_to: date_to
            },
    ...
    $(document).ready(function() {
        $("a").click(function() {
            var date_from = $("#date_from").val()
            var date_to = $("#date_to").val()
            var url = "<?php echo base_url(); ?>/quickly/dataDownload?date_from=" + date_from + "&date_to=" + date_to;
            // alert(url)
